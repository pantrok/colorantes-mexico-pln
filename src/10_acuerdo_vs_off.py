...
def main() -> None:
    ...
    nuestros = {norma(t) for t, _, _ in terminos_ordenados(dic)}
    ...
    if ruta.exists():
        ...
        """).df()
        buscar = [(r.forma_oficial, norma(r.forma_oficial), r.sin, r.codigo_e, r.clase)
                  for r in faltan.itertuples() if len(norma(r.forma_oficial)) >= 5]
        pat = {t: re.compile(r"\b" + re.escape(t) + r"\b") for _, t, _, _, _ in buscar}
        enc = []
        for row in df.itertuples(index=False):
            texto = normalizar(row.ingredientes_texto)
            tags = {str(a).replace("en:", "").upper()
                    for a in como_lista(row.aditivos_tags)}
            for orig, t, sin, e, cl in buscar:
                if pat[t].search(texto):
                    enc.append({"code": row.code, "producto": row.nombre_producto,
                                "forma_oficial": orig, "sin": sin, "codigo_e": e,
                                "clase": cl, "ya_en_tags": bool(e and e in tags)})
    ...
    if conteo:
        print(f"\n  productos con formas legales que hoy NO reconocemos: "
